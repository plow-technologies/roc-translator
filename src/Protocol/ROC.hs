...
import Data.List                   ()
import Data.IntMap.Strict          (fromList,minViewWithKey,singleton,size,Key,IntMap,foldrWithKey)
...
testQuery :: IO (Either String RocOpCodeResponse)
testQuery = do
  doubleEither <- requestOpCodeTableData testHostAddress testModemConfig testRocAccessConfig `traverse` makeOpCodeTable
  return $ join doubleEither
...
makeOpCodeTable :: Either String (OpCodeTable (TlpData ()))
...
makeDataMap :: Either Text (OpCodeData (TlpData ()))
makeDataMap = makeOpCodeData myDataMap

myDataMap :: IntMap (TlpData ())
myDataMap = fromList [( 1  , TlpUnsignedInt8 (Left ())),
